...
export const InvokeFunctionOrLoading = () => {
    ...
    const [folderURI, setFolderURI] = React.useState(undefined);
    ...
    function handleMessage(message) {
        if (message.data.action === 'invoke') {
            setName(_ => message.data.name);
            setId(_ => message.data.id);
            setFolderURI(_ => message.data.uri);
            setInstance(_ => message.data.instance);
            setInvokeURL(_ => message.data.invokeURL);
            setTemplate(_ => message.data.template);
            setBasicTemplates(_ => message.data.basicTemplates);
            setLoading(_ => false);
        }
    }

    function handleInvokeSubmit(
        name: string,
        instance: string,
        id: string,
        path: string,
    ...
    return (
        <InvokeFunction
            ...
            onInvokeSubmit={handleInvokeSubmit}
        />
    );
};

export class InvokeFunction extends React.Component<
    InvokeFunctionPageProps,
    {
        id: string;
        instance: string;
    ...
    handleBtnDisable = (): boolean => {
        return (
            ...
            (this.state.mode === 'text'
                ? this.state.input.length === 0
                : this.state.inputFilePath.length === 0)
        );
    };

    invokeFunction = (): void => {
        this.props.onInvokeSubmit(
            this.props.name,
            this.state.instance,
            this.state.id,
            this.props.uri.fsPath,
            this.state.contentType,
            this.convert(this.state.format),
            this.state.source,
            this.state.type,
            this.state.input,
            this.state.inputFilePath,
            this.state.enableInvokeURL,
            this.state.invokeURL,
        );
    };
                    ...
                    <Box display='flex' flexDirection={'column'}>
                        <Stack direction='column' spacing={2} margin={5}>
                                        ...
                                        <Stack direction='column' spacing={2}>
                                            ...
                                            >
                                                ...
                                                <TextField
                                                    ...
                                                    onChange={(e) =>
                                                        this.setValue(e.target.value, 'invokeURL')
                                                    }
                                                />
                                            </Stack>
                                        </Stack>
                            ...
                            >
                                <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                                    <Typography>
                                        Data (content) for this request. (default 'Hello World')
                                    </Typography>
                                </AccordionSummary>
                                <AccordionDetails>
                                    <Stack direction='column' spacing={2}>
                                        ...
                                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                                            ...
                                            {mode === 'text' ? (
                                                <TextField
                                                    ...
                                                    onChange={(e) =>
                                                        this.setValue(e.target.value, 'input')
                                                    }
                                                    error={input.length === 0}
                                                    placeholder='Data to send in the request'
                                                    helperText={
                                                        input.length === 0 ? 'Required data' : ''
                                                    }
                                                />
                                            ) : (
                                                <>
                            ...
                            <Stack direction='column'>
                                <Button
                                    ...
                                    className='buttonStyle'
                                    style={{
                                        backgroundColor: this.handleBtnDisable()
